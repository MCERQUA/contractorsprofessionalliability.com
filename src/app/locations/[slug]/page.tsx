...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${loc.blurb} E&O, workers' comp, general liability, commercial auto, and professional liability for ${loc.name} professional contractors. 15-minute quotes.`,
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              ...
              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img src="/images/coverage.jpg" alt={`Professional contractor in ${loc.name}`} className="w-full h-[300px] md:h-[360px] object-cover" loading="lazy" />
                </div>
              </FadeIn>
            </div>
          </div>
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              ...
              <h2 className="mt-3 h-section">The full program, built for {loc.name} professional contractors.</h2>
              <p className="mt-4 lead">From a single-operator firm to a multi-project design-build group, we coordinate every line a {loc.name} professional contractor needs.</p>
            </FadeIn>
        ...
        <FAQ items={locFaqs} eyebrow={`${loc.name} — FAQ`} title={<>Professional liability questions for <span className="text-clay">{loc.name}</span> contractors</>} background="sand" />

        <CTABand title={`Insuring professional contractors in ${loc.name} since 2005`} description={`Local knowledge, A-rated markets, and 15-minute quotes. Call ${SITE.phone} or request a quote online.`} />
      </main>
      <Footer />
    </>
  );
